...
-- One-off/idempotent cleanup after moving from schedule-level inventory
-- to concrete start-time slot inventory.
-- Safe to run after doctor and appointment Flyway migrations have completed.
...
update appointment.appointment a
...
delete from appointment.slot_inventory inv
where not exists (
    select 1
    from doctor.doctor_schedule_time_slot ts
...
from doctor.doctor_schedule_time_slot ts
left join appointment.appointment a on a.schedule_id = ts.id
