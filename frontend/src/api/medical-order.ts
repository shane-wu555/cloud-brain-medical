...
export async function uploadAttachment(id: string, file: File) { const data = new FormData(); data.append('file', file); return (await http.post<MedicalAttachment>(`/medical-orders/${id}/attachments`, data, { timeout: 10 * 60 * 1000 })).data }
...
export async function submitCt(id: string, attachmentId: string) { return (await http.post<AiMedicalTask>(`/medical-orders/${id}/ct-analysis`, { attachmentId }, { timeout: 10 * 60 * 1000 })).data }
export async function refreshAiTask(taskId: string) { return (await http.get<AiMedicalTask>(`/medical-orders/ai-tasks/${taskId}`, { timeout: 2 * 60 * 1000 })).data }
