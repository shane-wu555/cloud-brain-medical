    ...
    <div class="workspace">
      <header class="topbar">
        <div>
          ...
          <h1>今日接诊队列</h1>
          <p class="muted">查看待接诊患者，维护病历、医嘱、报告和处方。</p>
        </div>
          ...
          <section class="grid">
            <el-card class="span-7" shadow="never">
              ...
              <el-table :data="filteredAppointments" highlight-current-row @current-change="selectAppointment">
                <el-table-column prop="queueNumber" label="序号" width="70" />
                ...
                <el-table-column label="操作" width="190">
                  <template #default="{ row }">
                    ...
                    <el-button size="small" link @click.stop="skip(row)">过号</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </el-card>
            ...
            <el-card class="span-5" shadow="never">
              ...
              <el-form label-position="top">
                ...
                <el-form-item label="初步诊断"><el-input v-model="recordForm.diagnosis" /></el-form-item>
                <el-form-item label="治疗方案"><el-input v-model="recordForm.treatmentPlan" type="textarea" :rows="3" /></el-form-item>
                <el-form-item label="AI 修订说明"><el-input v-model="recordForm.doctorRevisionNote" /></el-form-item>
            ...
            <el-card class="span-12" shadow="never">
              ...
              <div class="toolbar">
                ...
                <el-select v-model="itemCategory" clearable placeholder="项目分类" style="width:160px">
                  <el-option label="检查" value="CHECK" />
                  <el-option label="检验" value="LAB" />
                  <el-option label="处置" value="DISPOSAL" />
                </el-select>
                <el-select v-model="orderUrgency" style="width:130px">
                  <el-option label="常规" value="ROUTINE" />
                  <el-option label="急诊" value="EMERGENCY" />
                </el-select>
                <el-button type="primary" :disabled="!current || !selectedItems.length" @click="submitOrders">
            ...
            <el-card class="span-12" shadow="never">
              <template #header>已确认医技报告</template>
              <el-table :data="formalReports">
                ...
                <el-table-column prop="createdByType" label="来源" width="90" />
              </el-table>
            </el-card>

            <el-card class="span-12" shadow="never">
              <template #header>本次及历史病历</template>
              <div class="toolbar">
                <el-input v-model="historyReason" placeholder="历史病历访问原因" />
                <el-button :disabled="!current" @click="loadHistory">查看历史</el-button>
              </div>
        ...
        <aside class="ai-pane">
          <el-card shadow="never" class="ai-card">
            <template #header>
              <div class="ai-header">
                <span>AI 助理医生</span>
                <el-tag :type="aiFallback ? 'warning' : aiModel ? 'success' : 'info'" effect="plain">
                  {{ aiModelLabel }}
                </el-tag>
              </div>
            </template>
            <el-alert title="AI 结果不能直接形成诊断或处方" type="warning" :closable="false" show-icon />
            ...
            <div class="ai-messages">
              ...
              <el-empty v-if="!aiMessages.length" description="生成建议后在此显示" :image-size="72" />
            </div>
...
import { createPrescription, getDrugs, type Drug } from '../../api/pharmacy';
...
const filteredAppointments = computed(() => {
  ...
  return keyword ? appointments.value.filter((item) => `${item.patientName}${item.businessNo}`.toLowerCase().includes(keyword)) : appointments.value;
});
...
async function selectAppointment(row?: Appointment) {
  ...
  Object.assign(recordForm, {
    chiefComplaint: row?.triageSummary ?? '',
    presentIllness: '',
    pastHistory: '',
    allergyHistory: '',
    physicalExamination: '',
...
async function skip(appointment: Appointment) { await skipAppointment(appointment.id); ElMessage.success('已顺延'); await loadQueue(); }
...
function applyDiagnosis(message: { id: string; content: string }) {
  ...
  recordForm.doctorRevisionNote = '医生已复核 AI 诊断草稿';
...
<style scoped>
.doctor-split { display: grid; grid-template-columns: minmax(0, 2fr) minmax(340px, 1fr); gap: 16px; align-items: start; }
.business-pane { min-width: 0; }
.ai-pane { position: sticky; top: 20px; }
.ai-card { border-color: #c7d2fe; }
.ai-header, .toolbar { display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.toolbar { margin-bottom: 12px; }
.context-block { margin: 16px 0; padding: 12px; border-radius: 8px; background: #f8fafc; }
.context-block p, .ai-message p, .rx-item p { margin: 6px 0 0; }
.ai-messages { min-height: 220px; max-height: 360px; overflow: auto; margin-bottom: 12px; }
.ai-message, .rx-item { margin-bottom: 12px; padding: 12px; border-left: 3px solid #6366f1; background: #eef2ff; }
.rx-item { border-left-color: #16a34a; background: #f0fdf4; }
.ai-message span { font-weight: 700; }
.ai-action { margin-top: 10px; }
.rx-panel { display: grid; gap: 10px; }
@media (max-width: 1100px) { .doctor-split { grid-template-columns: 1fr; } .ai-pane { position: static; } }
</style>
