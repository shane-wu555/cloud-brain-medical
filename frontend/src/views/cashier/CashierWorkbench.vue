    ...
    <section class="grid">
      ...
      <el-card class="span-7" shadow="never"><template #header>现场挂号收费</template>
        <el-form label-position="top"><el-form-item label="排班时间"><el-select v-model="scheduleId" class="full">
          <el-option v-for="item in scheduleOptions" :key="item.slot.id" :disabled="item.slot.available<=0" :label="`${item.schedule.doctorName} · ${item.schedule.workDate} ${item.schedule.period} ${item.slot.startTime.slice(0,5)} · 剩余 ${item.slot.available}`" :value="item.slot.id" />
...
onMounted(async()=>{departments.value=await getDepartments();schedules.value=await getSchedules();scheduleId.value=scheduleOptions.value.find(i=>i.slot.available>0)?.slot.id??''})
</script>
