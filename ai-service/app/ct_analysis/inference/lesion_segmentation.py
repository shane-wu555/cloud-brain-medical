...
_REPO_MODEL = str(
    ...
    / "models"
    / "lesion_segmentation"
    / "lesion_segmentation.onnx"
)
_DEFAULT_MINIO_OBJECT = "models/lesion_segmentation/lesion_segmentation.onnx"
_DEFAULT_LOCAL_CACHE = "/tmp/ct_models/lesion_segmentation.onnx"
...
def segment_lesion(
    ...
    return {
        "enabled": True,
        "threshold": threshold,
        "hasLesionRegion": bool(top_slices),
...
def lesion_regions(seg_result: dict, label: str = "lesion") -> list[dict]:
    """Convert segmentation top slices to abnormal region records."""
    regions: list[dict] = []
    for item in seg_result.get("topSlices", []):
        regions.append(
            {
                "sliceIndex": int(item["sliceIndex"]),
                "bbox": item["bbox"],
                "label": label,
                "confidence": round(float(item.get("maxProb", 0.0)), 4),
                "source": "lesion_segmentation",
                "areaRatio": item.get("areaRatio", 0.0),
            }
        )
    return regions
...
def _resolve_model_path() -> str | None:
    local_path = os.getenv("CT_LESION_SEGMENTATION_MODEL", _DEFAULT_LOCAL_CACHE)
    ...
    try:
        ...
        log.warning(f"Lesion segmentation model unavailable: {exc}")
        return None
...
def _fallback_result(enabled: bool = False) -> dict:
    return {
        "enabled": enabled,
        "threshold": float(os.getenv("CT_LESION_SEG_THRESHOLD", "0.35")),
