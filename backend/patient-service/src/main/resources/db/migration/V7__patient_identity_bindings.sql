-- Apply the patient identity/binding model after the existing remote V6 migration.
-- This migration is intentionally idempotent because some environments already
-- validated V6 with the same SQL text while others have not applied it.
...
begin
    if exists (
        ...
        where table_schema = 'patient'
          and table_name = 'patient_profile'
...
drop index if exists patient.uq_patient_profile_account_certificate;
drop index if exists patient.idx_patient_profile_account;
