...
create index if not exists idx_patient_id_card on patient(id_type, id_number) where id_number is not null;
create index if not exists idx_patient_phone   on patient(phone)               where phone    is not null;
...
create index if not exists idx_binding_account on account_binding(account_id);
create index if not exists idx_binding_patient on account_binding(patient_id);
