...
insert into department (id, name, description) values
    ('dept-neuro',   '神经内科', '头痛、眩晕、脑血管疾病'),
    ('dept-imaging', '检查科',   'B超、CT/MRI 等检查'),
    ('dept-general', '全科医学', '常见病与慢病复诊')
on conflict do nothing;

-- 初始三位医生，直接赋予 8 位工号
-- BCrypt hash 对应密码: abc12345
insert into doctor (id, name, title, department_id, role_type, specialty, employee_no) values
    ('doctor-001', '张医生', '主任医师',   'dept-neuro',   'OUTPATIENT_DOCTOR', '头痛与脑血管疾病', '00010001'),
    ('doctor-002', '李医生', '副主任医师', 'dept-imaging', 'CHECK_DOCTOR',      '头部 CT/MRI 影像检查', '00030001'),
    ('doctor-003', '陈医生', '主治医师',   'dept-general', 'OUTPATIENT_DOCTOR', '慢病管理', '00040001')
on conflict do nothing;
...
insert into doctor_schedule (id, doctor_id, department_id, work_date, period, capacity) values
    ...
    ('schedule-004', 'doctor-003', 'dept-general', current_date,     '全天', 30)
on conflict do nothing;
