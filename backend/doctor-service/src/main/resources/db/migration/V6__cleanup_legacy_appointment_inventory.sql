do $$
begin
    if to_regclass('appointment.appointment') is null
       or to_regclass('appointment.slot_inventory') is null then
        return;
    end if;
    ...
    with appointment_slot_mapping as (
        ...
        join doctor_schedule s on s.id = a.schedule_id
        join lateral (
            select ts.id, ts.start_time
            from doctor_schedule_time_slot ts
