...
public class SecurityConfig {
 ...
 @Bean SecurityFilterChain chain(HttpSecurity h,JwtAuthenticationConverter c)throws Exception{return h.csrf(x->x.disable()).sessionManagement(x->x.sessionCreationPolicy(SessionCreationPolicy.STATELESS)).authorizeHttpRequests(x->x.requestMatchers("/actuator/health","/api/internal/**").permitAll().anyRequest().authenticated()).oauth2ResourceServer(x->x.jwt(j->j.jwtAuthenticationConverter(c))).build();}
}
