...
public class BaseCatalogController {
    ...
    @GetMapping("/medical-items") public List<MedicalItem> medicalItems(@RequestParam(required=false) String category) {
        return jdbc.query("select code,name,category,price from medical_item_catalog where active and (? is null or category=?) order by category,name",
                (rs,row)->new MedicalItem(rs.getString(1),rs.getString(2),rs.getString(3),rs.getBigDecimal(4)),category,category);
    }
