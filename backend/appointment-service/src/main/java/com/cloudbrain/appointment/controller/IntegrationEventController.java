...
public class IntegrationEventController {
    ...
    @GetMapping public List<MedicalRecordEventRepository.EventView> list(@RequestParam(required=false) String status){return repository.findEvents(status);}
    @PostMapping("/{id}/retry") public void retry(@PathVariable String id){repository.retry(id);}
}
