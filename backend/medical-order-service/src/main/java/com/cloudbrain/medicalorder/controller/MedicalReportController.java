...
public class MedicalReportController {
    ...
    public AiMedicalTask task(@PathVariable("taskId") String taskId,JwtAuthenticationToken auth){return service.refresh(taskId,auth.getToken().getSubject());}
    @PostMapping("/{orderId}/reports/draft") @PreAuthorize("hasAnyRole('CHECK_DOCTOR','LAB_DOCTOR','DISPOSAL_DOCTOR')")
    public MedicalReport draft(@PathVariable("orderId") String orderId,@RequestBody ReportRequest request,JwtAuthenticationToken auth){return service.manualDraft(orderId,request.findings(),request.conclusion(),request.advice(),auth.getToken().getSubject());}
    @PostMapping("/{orderId}/reports/confirm") @PreAuthorize("hasAnyRole('CHECK_DOCTOR','LAB_DOCTOR','DISPOSAL_DOCTOR')")
    public MedicalReport confirm(@PathVariable("orderId") String orderId,@RequestBody ReportRequest request,JwtAuthenticationToken auth){return service.confirm(orderId,request.findings(),request.conclusion(),request.advice(),auth.getToken().getSubject(),auth.getToken().getClaimAsString("role"));}
    @PostMapping("/{orderId}/reports/reject") @PreAuthorize("hasAnyRole('CHECK_DOCTOR','LAB_DOCTOR','DISPOSAL_DOCTOR')")
    public MedicalReport reject(@PathVariable("orderId") String orderId,@RequestBody RejectRequest request,JwtAuthenticationToken auth){return service.reject(orderId,request.reason(),auth.getToken().getSubject(),auth.getToken().getClaimAsString("role"));}
    @GetMapping("/reports") @PreAuthorize("hasAnyRole('PATIENT','OUTPATIENT_DOCTOR','CHECK_DOCTOR','LAB_DOCTOR','DISPOSAL_DOCTOR')")
    public List<MedicalReport> reports(@RequestParam(name="patientId", required=false)String patientId,JwtAuthenticationToken auth){return service.list(patientId,auth.getToken().getSubject(),auth.getToken().getClaimAsString("role"));}
