...
public class LaboratoryWorkflowRepository {
    ...
    public boolean isLabOrder(String orderId) {
        Integer count = jdbc.queryForObject("select count(*) from medical_order where id = ?::uuid and order_type = 'LAB'", Integer.class, orderId);
        return count != null && count == 1;
    }

    public Specimen createSpecimen(String orderId, String type, String barcode) {
        String id = UUID.randomUUID().toString();
        jdbc.update("insert into specimen(id, order_id, specimen_type, barcode) values (?::uuid, ?::uuid, ?, ?)",
                id, orderId, type, barcode);
        return specimen(id).orElseThrow();
    }

    public Optional<Specimen> specimen(String id) {
        return jdbc.query("select * from specimen where id = ?::uuid", (rs, n) -> new Specimen(
                rs.getString("id"), rs.getString("order_id"), rs.getString("specimen_type"),
                rs.getString("barcode"), rs.getString("status"), null,
                time(rs.getTimestamp("collected_at")), time(rs.getTimestamp("received_at")),
                time(rs.getTimestamp("analyzed_at")), time(rs.getTimestamp("completed_at")),
    ...
    public List<Specimen> specimens(String orderId) {
        return jdbc.query("select * from specimen where order_id = ?::uuid order by created_at", (rs, n) -> new Specimen(
                rs.getString("id"), rs.getString("order_id"), rs.getString("specimen_type"),
                rs.getString("barcode"), rs.getString("status"), null,
                time(rs.getTimestamp("collected_at")), time(rs.getTimestamp("received_at")),
                time(rs.getTimestamp("analyzed_at")), time(rs.getTimestamp("completed_at")),
    ...
    public boolean transition(String id, String current, String next, String actorId, String reason) {
        String timestampColumn = switch (next) {
            ...
            case "ANALYZING" -> "analyzed_at";
            case "REVIEWED" -> "completed_at";
        ...
        String sql = "update specimen set status = ?, " + timestampColumn + " = now(), "
                + "discard_reason = case when ? = 'DISCARDED' then ? else discard_reason end where id = ?::uuid and status = ?";
        return jdbc.update(sql, next, next, reason, id, current) == 1;
    }
    ...
    public LaboratoryResultItem upsertResult(String orderId, String specimenId, String itemCode, String itemName,
        ...
        String id = UUID.randomUUID().toString();
        jdbc.update("""
                insert into lab_result_item
                    (id, order_id, specimen_id, item_code, item_name, result_value, unit,
    ...
    public List<LaboratoryResultItem> results(String orderId) {
        return jdbc.query("select * from lab_result_item where order_id = ?::uuid order by item_code", (rs, n) -> new LaboratoryResultItem(
                rs.getString("id"), rs.getString("order_id"), rs.getString("specimen_id"),
