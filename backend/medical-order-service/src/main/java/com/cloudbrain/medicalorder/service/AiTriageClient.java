...
public class AiTriageClient {
    ...
    public TriageResult triage(MedicalOrder order,List<ExecutorCandidate> candidates){
        List<Map<String,Object>> payloadCandidates=candidates.stream().map(c->Map.<String,Object>of(
                "workspaceId",c.id(),"workspaceName",c.name(),"doctorId",c.id(),"doctorName",c.name(),
                "specialties",Arrays.asList(c.specialties().split(",")),
        ...
        try{
            ...
            String workspaceId = (String) result.getOrDefault("workspaceId", result.get("doctorId"));
            String workspaceName = (String) result.getOrDefault("workspaceName", result.get("doctorName"));
            return new TriageResult(workspaceId, workspaceName, (String)result.get("location"),
