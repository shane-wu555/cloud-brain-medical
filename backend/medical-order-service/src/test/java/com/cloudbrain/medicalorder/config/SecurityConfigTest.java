...
class SecurityConfigTest {
    ...
    @Test
    void jwtDecoderBuildsForValidSecret() {
        assertThat(config.jwtDecoder("12345678901234567890123456789012", "issuer")).isNotNull();
    }

    @Test
    void converterMapsRoleClaimToAuthority() {
        Jwt jwt = Jwt.withTokenValue("token")
                .header("alg", "none")
                .claim("role", "CHECK_DOCTOR")
                .build();

        var authentication = config.jwtAuthenticationConverter().convert(jwt);

        assertThat(authentication.getAuthorities()).extracting("authority").containsExactly("ROLE_CHECK_DOCTOR");
    }
    ...
    void securityFilterChainBuildsWithHttpSecurity() throws Exception {
        ...
        assertThat(config.securityFilterChain(http, config.jwtAuthenticationConverter())).isSameAs(chain);
    }
}
