...
class AuditPublisherTest {
    ...
    void publishSerializesAuditEventWithActorAndIp() throws Exception {
        ...
        AuditPublisher publisher =
                new AuditPublisher(rabbitTemplate, objectMapper, "audit.exchange", "audit.event", "medical-order-service");

        publisher.publish("CREATE", "MEDICAL_ORDER", "order-1", "patient-1", "biz-1", "user-1", "CHECK_DOCTOR", Map.of("k", "v"));

        ArgumentCaptor<String> payloadCaptor = ArgumentCaptor.forClass(String.class);
        verify(rabbitTemplate).convertAndSend(eq("audit.exchange"), eq("audit.event"), payloadCaptor.capture());

        JsonNode payload = objectMapper.readTree(payloadCaptor.getValue());
        assertThat(payload.path("service").asText()).isEqualTo("medical-order-service");
    ...
    void publishSwallowsBrokerFailure() {
        ...
        publisher.publish("CREATE", "MEDICAL_ORDER", "order-1", "patient-1", "biz-1", "user-1", "CHECK_DOCTOR", Map.of());
    }
}
